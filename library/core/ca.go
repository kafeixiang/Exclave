...
func extractOrReadMozillaCAPem() ([]byte, error) {
	...
	pemInternal.Close()
	if b, err := os.ReadFile(internalAssetsPath + mozillaIncludedPem); err == nil && bytes.Equal(b, pemBytes) {
		return pemBytes, nil
	}
	if err := os.MkdirAll(internalAssetsPath, 0755); err != nil {
		return nil, err
	}
	pemFile, err := os.Create(internalAssetsPath + mozillaIncludedPem) // for plugins
	if err != nil {
		return nil, err
	}
	defer pemFile.Close()
