        ...
        "hysteria2" -> {
            return listOf(Hysteria2Bean().apply {
                ...
                serverPorts = ports?.joinToString(",") {
                    if (it.first == it.second) it.first.toString() else it.first.toString() + "-" + it.second.toString()
                } ?: port.toString()
...
private fun String.toIntRanges(): List<Pair<Int, Int>> {
...
private fun String.toIntRange(): Pair<Int, Int>? {
    val s = this.trim()
    if (s.isEmpty()) {
        return Pair(0, 0)
    }
