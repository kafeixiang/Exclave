...
class MatsuriJSInterface(val plgId: String) {
                ...
                runOnIoDispatcher {
                    ...
                    try {
                        ...
                        if (!obj.getBoolean("ok")!!) {
                            throw Exception("plugin refuse to run: ${obj.getStr("reason")}")
                        }
    ...
    private suspend fun nekoInit(): String {
        ...
        sendData.set("lang", Locale.getDefault().toLanguageTag())
        sendData.set("plgId", plgId)
        sendData.set("managerVersion", MatsuriPluginManager.managerVersion)
    ...
    inner class NekoProtocol(val protocolId: String, val callJS: suspend (String) -> String) {
        ...
        suspend fun buildAllConfig(
            ...
            val sendData = JSONObject()
            sendData.set("port", port)
            sendData.set(
                "sharedStorage",
                Base64.encodeUrlSafe(bean.sharedStorage.toString().toByteArray())
            )
            otherArgs?.forEach { (t, u) -> sendData.set(t, u) }

            return callProtocol(
                "buildAllConfig", Base64.encodeUrlSafe(sendData.toString().toByteArray())
            )
        }

        suspend fun parseShareLink(shareLink: String): String {
            val sendData = JSONObject()
            sendData.set("shareLink", shareLink)

            return callProtocol(
                "parseShareLink", Base64.encodeUrlSafe(sendData.toString().toByteArray())
            )
        }
        ...
        suspend fun setSharedStorage(sharedStorage: String) {
            callProtocol(
                "setSharedStorage",
                Base64.encodeUrlSafe(sharedStorage.toByteArray())
            )
        }
        ...
        suspend fun onPreferenceChanged(key: String, v: Any) {
            val sendData = JSONObject()
            sendData.set("key", key)
            sendData.set("newValue", v)

            callProtocol(
                "onPreferenceChanged",
                Base64.encodeUrlSafe(sendData.toString().toByteArray())
            )
        }
    ...
    inner class JsObject {
        ...
        fun setMenu(key: String, entries: String) {
            runOnMainDispatcher {
                preferenceScreen?.findPreference<SimpleMenuPreference>(key)?.apply {
                    MatsuriPreferenceInflater.setMenu(this, JSONObject(entries))
                }
            }
        }
        ...
        fun getKV(type: Int, key: String): String {
            ...
            try {
                when (type) {
                    0 -> v.set("v", DataStore.profileCacheStore.getBoolean(key))
                    1 -> v.set("v", DataStore.profileCacheStore.getFloat(key))
                    2 -> v.set("v", DataStore.profileCacheStore.getInt(key))
                    3 -> v.set("v", DataStore.profileCacheStore.getLong(key))
                    4 -> v.set("v", DataStore.profileCacheStore.getString(key))
                }
            } catch (e: Exception) {
                Logs.e("getKV: $e")
            }
