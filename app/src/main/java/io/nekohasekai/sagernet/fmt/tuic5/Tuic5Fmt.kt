...
fun parseTuic(server: String): AbstractBean {
    ...
    if (!version.isNullOrEmpty() && version == "4" || link.password.isEmpty()) {
        return TuicBean().apply {
            ...
            link.queryParameter("congestion-control")?.let {
                ...
                if (it == "new-reno" || it == "new_reno") {
                    congestionController = "new_reno"
                }
            }
            link.queryParameter("udp-relay-mode") ?:
            link.queryParameter("udp_relay_mode") ?:
            link.queryParameter("udp-relay_mode") ?:
            link.queryParameter("udp_relay-mode")?.takeIf { it == "native" || it == "quic" }?.let {
                udpRelayMode = it
            }
            link.queryParameter("disable_sni") ?:
            link.queryParameter("disable-sni")?.takeIf { it == "1" || it == "true" }?.let {
                disableSNI = true
            }
            link.queryParameter("reduce_rtt") ?:
            link.queryParameter("reduce-rtt")?.takeIf { it == "1" || it == "true" }?.let {
                reduceRTT = true
            }
            link.fragment?.let {
                name = it
    ...
    return Tuic5Bean().apply {
        ...
        link.queryParameter("allow-insecure") ?:
