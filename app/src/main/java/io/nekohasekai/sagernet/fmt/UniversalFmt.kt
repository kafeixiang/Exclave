...
fun ProxyGroup.exportBackup(): String {
    export = true
    val link = "subscription?" + Base64.encode(KryoConverters.serialize(this).zlibCompress(9))
    export = false
    return link
}

fun parseGroupBackup(text: String): ProxyGroup {
    val type = text.substringBefore("?")
    if (type != "subscription") error("Type $type not found")
    val data = text.substringAfter("?")
    if (data.isEmpty()) {
        error("data is empty")
    }
    return KryoConverters.deserialize(
        ...
        Base64.decode(text.substringAfter("?")).zlibDecompress()
    ).apply {
        export = false
    }
}
